...
class SparkHitLevelJob:
    ...
    def _sort_by_time(self, df: DataFrame) -> DataFrame:
        return df.withColumn('hit_time_gmt', F.col('hit_time_gmt').cast('long')) \
    ...
    def _parse_referrers(self, df: DataFrame) -> DataFrame:
        ...
        return (
            df.withColumn('se_domain', parsed['se_domain'])
              .withColumn('se_keyword', parsed['se_keyword'])
        )
    ...
    def _propagate_referrers(self, df: DataFrame) -> DataFrame:
        ...
        return (
            df.withColumn('se_domain', F.last('se_domain', ignorenulls=True).over(window))
              .withColumn('se_keyword', F.last('se_keyword', ignorenulls=True).over(window))
        )
